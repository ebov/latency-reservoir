...
```js 
const combinedLogs = FileAttachment(`/analyses/BEAST/${observable.params.dir}/combinedLogs/${observable.params.stem}.combined.log`).tsv({typed:true})
```
```js 
const treeFile = FileAttachment(`/analyses/BEAST/${observable.params.dir}/combinedLogs/${observable.params.stem}.combined.trees`).stream()
```
